...
abstract class handler {
    ...
    protected function validate_field(field_controller $field): field_controller {
        $categories = $this->get_categories_with_fields();
        $category = $this->get_category_from_array($categories,  $field->get('categoryid'), $this->get_component(),
            $this->get_area(), $this->get_itemid());
    ...
    public function instance_form_definition(\MoodleQuickForm $mform, int $instanceid = 0,
        ...
        $editablefields = $this->get_editable_fields($instanceid);
        $fieldswithdata = api::get_instance_fields_data($editablefields, $instanceid, true, $this->get_component(), $this->get_area()); // TODO add itemid?
        $lastcategoryid = null;
